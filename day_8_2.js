...
const parse = data => data
    ...
    .map(line => line.match(/^([a-z]+) (inc|dec) (-?\d+) if (([a-z]+).*)/))
    .filter(line => line)
    .map(match => ({
        register: match[1],
        what: match[2] === "inc" ? "+" : "-",
...
const run = data => {
    ...
    is.forEach(i => {
        const code = `if (${i.condition}) { ${i.register} ${i.what}= ${i.amount} }`
        ...
        console.log()
        */
        const this_max = Math.max.apply(null, Object.values(sandbox))
        max = (max === null) ? this_max : Math.max(this_max, max);
    })
