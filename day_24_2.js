...
const run = raw => {
    ...
    while (states.length) {
        ...
        if (nexts.length === 0) {
            if (state.bridge.length === max_length) {
                longests.push(state)
            } else if (state.bridge.length > max_length) {
                max_length = state.bridge.length 
                longests = [ state ]
                console.log("*", "longest", max_length)
            }
        }
    ...
    return Math.max.apply(null, longests.map(l => l.strength))
}
